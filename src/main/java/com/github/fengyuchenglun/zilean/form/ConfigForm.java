package com.github.fengyuchenglun.zilean.form;

import com.github.fengyuchenglun.zilean.AppContext;
import com.github.fengyuchenglun.zilean.form.component.PackageChooseTextField;
import com.github.fengyuchenglun.zilean.generator.MybatisPlusGeneratorHelper;
...
import com.github.fengyuchenglun.zilean.util.DbUtils;
...
import static com.github.fengyuchenglun.zilean.constant.Constants.*;
...
public class ConfigForm extends JFrame {
    ...
    public ConfigForm(String name, AnActionEvent action) throws HeadlessException {
        ...
        AppContext.getInstance().setConfigForm(this);
        AppContext.getInstance().setProject(this.project);
    ...
    private void initData() {
        ...
        AppContext.getInstance().setTableMap(tables.parallelStream().collect(Collectors.toMap(DasObject::getName, i -> i)));
        AppContext.getInstance().setTableList(Collections.list(tableNames.elements()));
        // 设置表列表
        ...
        // 重置按钮是否显示
        if (project != null) {
            AppContext.getInstance().setBasePath(Objects.requireNonNull(project.getBasePath()));
            AppContext.getInstance().setBaseXmlPath(Objects.requireNonNull(project.getBasePath()).concat(DEFAULT_MAPPER_PATH));
            String dataCacheFile = AppContext.getInstance().getDataCacheFile();
    ...
    private String getTablePrefix(List<String> selectedTableList) {
        String tablePrefix = "";
        if (CollectionUtils.isNotEmpty(selectedTableList)) {
            String selectedTable = selectedTableList.get(0);
            int index = StringUtils.indexOf(selectedTable, "_");
            if (index >= 0) {
                tablePrefix = selectedTable.substring(0, index + 1);
            }
        }
        return tablePrefix;
    }
    ...
    private void reset() {
        ...
        this.tablePrefixField.setText(this.getTablePrefix(this.tableList.getSelectedValuesList()));
        ...
        if (project != null) {
            this.basePathField.setText(Objects.requireNonNull(project.getBasePath()).concat(DEFAULT_SOURCE_PATH));
            this.xmlPathField.setText(Objects.requireNonNull(project.getBasePath()).concat(DEFAULT_MAPPER_PATH));
        }
    ...
    private void bindListeners() {
        ...
        this.basePathField.addActionListener(e -> {
            FileChooser.chooseFile(folderChooser, project, this, null, x -> {
                ...
                AppContext.getInstance().setBasePath(basePath);
                AppContext.getInstance().setBaseXmlPath(baseXmlPath);
                baseXmlPath = getXmlPath(baseXmlPath, this.moduleNameField.getText());
        ...
        this.xmlPathField.addActionListener(e -> {
            FileChooser.chooseFile(folderChooser, project, this, null, x -> {
                ...
                AppContext.getInstance().setBaseXmlPath(x.getPath());
//                this.setAlwaysOnTop(true);
            });
        });

        this.selectBtn.addActionListener(e -> {
            List<String> tableList = AppContext.getInstance().getTableList();
        ...
        this.tableList.addListSelectionListener(x -> {
            List<String> list = ((JBList) x.getSource()).getSelectedValuesList();
            this.tablePrefixField.setText(this.getTablePrefix(list));
        });
        ...
        this.moduleNameField.getDocument().addDocumentListener(new DocumentListener() {
            ...
            public void insertUpdate(DocumentEvent e) {
                Thread.sleep(500);
                AppContext.getInstance().getConfigForm().updatePackagePath();
            }
    ...
    private void updatePackagePath() {
        ...
        String basePath = AppContext.getInstance().getBasePath();
        String baseXmlPath = basePath + DEFAULT_MAPPER_PATH;
        baseXmlPath = getXmlPath(baseXmlPath, moduleName);
        if (StringUtils.isNotBlank(moduleName)) {
            basePackage = DbUtils.joinPackage(parent, moduleName);
        }
        this.entityPackageField.setText(DbUtils.joinPackage(basePackage, "domain.entity"));
        this.dtoPackageField.setText(DbUtils.joinPackage(basePackage, "domain.dto"));
        this.voPackageField.setText(DbUtils.joinPackage(basePackage, "domain.model.vo"));
        this.queryPackageField.setText(DbUtils.joinPackage(basePackage, "domain.model.query"));
        this.formPackageField.setText(DbUtils.joinPackage(basePackage, "domain.model.query"));

        this.daoPackageField.setText(DbUtils.joinPackage(basePackage, "dao"));
        this.servicePackageField.setText(DbUtils.joinPackage(basePackage, "service"));
        this.serviceImplPackageField.setText(DbUtils.joinPackage(basePackage, "service.impl"));
        this.controllerPackageField.setText(DbUtils.joinPackage(basePackage, "controller"));
        this.xmlPathField.setText(baseXmlPath);
    }
    ...
    private Config buildConfig() {
        ...
        config.setTablePrefix(StringUtils.appendIfMissing(this.tablePrefixField.getText(), "_", "_"));
        ...
        AppContext.getInstance().setSelectedTableList(tableList.getSelectedValuesList());
        config.setTables(MybatisPlusGeneratorHelper.getInstance().getTableInfoList(config));
        return config;
    }
    ...
    private void generateCode() {
        ...
        MybatisPlusGeneratorHelper.getInstance().cacheGeneratorData(config);
        MybatisPlusGeneratorHelper.getInstance().generate(config);
        Messages.showMessageDialog(this, "代码生成完毕！", "提示", Messages.getInformationIcon());
        this.dispose();
    }

    private void loadConfigCache() {
        Config config = MybatisPlusGeneratorHelper.getInstance().readCacheData();
        if (config == null) {
            this.reset();
            return;
        }
        ...
        AppContext.getInstance().setBaseXmlPath(packageConfigs.getXml().getPkg());
    }

    private String getXmlPath(String baseXmlPath, String moduleName) {
        if (StringUtils.isBlank(baseXmlPath)) {
            return DEFAULT_MAPPER_PATH;
        }
        if (StringUtils.isBlank(moduleName)) {
            return baseXmlPath;
        }
        return baseXmlPath + UNIX_SEPARATOR + moduleName;
    }
